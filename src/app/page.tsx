...
export default function LandingPage() {
  ...
  const fadeIn = 5;
  const fadeOut = 8;
  const isLastBeat = beat.start >= 80;
  let opacity = 1;
  if (scrollPct < beat.start + fadeIn) {
    opacity = (scrollPct - beat.start) / fadeIn;
  } else if (!isLastBeat && scrollPct > beat.end - fadeOut) {
  ...
  return (
    ...
    <div ref={containerRef} style={{ height: "500vh", position: "relative" }}>
      ...
      <div style={stickyStyle}>
        ...
        {/* Buttons live outside blend mode so colors stay true */}
        {beat.showCta && (
          <div
            style={{
              position: "absolute",
...
const stickyStyle: React.CSSProperties = {
  position: "sticky",
  top: 0, // sticks to very top — covers behind navbar too
  height: "100vh", // full viewport height
  width: "100%",
  overflow: "hidden",
  marginTop: "-64px", // pull up by navbar height so it starts at true top
};
...
const textBlockStyle: React.CSSProperties = {
  ...
  zIndex: 3,
  paddingTop: "64px",
  mixBlendMode: "difference", // ← the magic line
  color: "white", // difference mode inverts white → dark on light bg, white on dark bg
};
